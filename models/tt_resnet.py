...
from tednet.tnn.tensor_train import TTConv2D, TTLinear
...
class TTResNet(nn.Module):
    def __init__(self, block, rs: Union[list, np.ndarray], num_blocks: list, num_classes:int):
        ...
        self.conv1 = TTConv2D([3, 1, 1], [2*c, 1*c, 1*c], [rs[0], rs[0], rs[0]], kernel_size=3, stride=1,
                                           padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.layer1 = self._make_layer(block,  [2*c, 1*c, 1*c],  [2*c, 1*c, 1*c], rs[1], num_blocks[0])
